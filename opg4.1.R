...
######Opgave 4.1#####
    output$distPlot <- renderPlot({
      ...
      ggplot(df_goal, aes(x = reorder(player.name, -Antal_mål), y = Antal_mål)) +
        ...
        labs(title = "Spillerne med flest mål",
             subtitle = "XXX",
             x = "Spiller",
             y = "Antal mål",
             caption = "Datakilde: WyScout",
             color = "Sum af xG") +
        scale_y_continuous(breaks = seq(min(0), max(ceiling(max(df_goal$Antal_mål) / 5) * 5), by = 5)) +
        coord_cartesian(ylim = c(0, ceiling(max(ceiling(max(df_goal$Antal_mål) / 5) * 5))))+
        theme_minimal() +
